                ...
                <div class="glass-card rounded-3xl p-6">
                    ...
                    <h3 class="font-bold mb-5 text-lg flex items-center gap-2">
                        ...
                        @if ($user->reputation >= 0)
                            <p
                                class="w-8 h-8 bg-green-500 rounded-full flex justify-center items-center font-bold text-white">
                                {{ $user->reputation }}</p>
                        @else
                            <p
                                class="w-8 h-8 bg-red-500 rounded-full flex justify-center items-center font-bold text-white">
                                {{ $user->reputation }}</p>
                        @endif
                    </h3>
                            ...
                            @foreach ($roommates as $mate)
                                <div class="text-center">
                                    ...
                                    @if ($mate->profile_image)
                                        <img src="{{ $mate->profile_image }}"
                                            class="w-16 h-16 rounded-full border-4 {{ $mate->is_admin == 1 ? 'border-yellow-500' : 'border-white' }} shadow-xl"
                                            onerror="this.src='{{ asset('image/shopping.png') }}'">
                                    @else
                                        <div
                                            class="w-16 h-16 rounded-full bg-indigo-500 flex items-center justify-center text-white text-3xl font-bold">
                                            {{ strtoupper(substr($mate->firstname, 0, 1) . substr($mate->lastname, 0, 1)) }}
                                        </div>
                                    @endif
